...
#初始化
layout,cell = IO.Start("guiopen")#在当前的图上继续画,如果没有就创建一个新的
layout.dbu = 0.001#设置单位长度为1nm
IO.pointdistance=500#设置腔的精度,转弯处相邻两点的距离

#画腔
painter3=CavityPainter(pya.DPoint(0,1050000),angle=-90,widout=48000,widin=16000,bgn_ext=0,end_ext=0)
c1bgninfo=painter3.Getinfo()[0:3]
c1bgninfo[2]+=180
def path1(painter):#设置内轮廓路径
    dl=547.0693762227893
    length=0
    length+=painter.Straight(570000)
...
anglelist1=zip([-90,90],[0, 0],range(2))
bgncinfo=[]
bgnlen=[0,0]
for angle1,angle2,index in anglelist1:
    ...
    length+=painter3.Draw(lambda painter:painter.Straight(500000-bgnlen[index]))
    length+=painter3.Draw(lambda painter:painter.Turning(40000,angle2))
    print("length of bgn %s : %s"%(index,length))
    print("add %s to bgnlen %s"%(length-500000,index))
    centerlineinfos.extend(painter3.centerlineinfos)
    regionlistout.extend(painter3.regionlistout)
    regionlistin.extend(painter3.regionlistin)
...
border=BasicPainter.DrawBorder(leng=4050000,siz=4050000,wed=50000)
BasicPainter.Output(cell,layer2,border)
