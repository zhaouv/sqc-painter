...
class ObjectPainter(object):
    ...
    def DrawBorder(self,wed=50000,leng=3050000,siz=3050000):
        ...
        for i in pts:
            i.x=-i.x
        self.outputlist.append(pya.DPolygon(pts))
    def DrawText(self,textstr,DCplxTrans,cell,layer1,layout):
        #左下角坐标,每个字宽0.6*倍数高0.7*倍数  
        #tr=pya.DCplxTrans(1,0,False,0,0)
        #倍数,逆时针度数,是否绕x翻转,平移x,平移y
        tr=pya.CplxTrans.from_dtrans(DCplxTrans)       
        param = { 
            "text": textstr, 
            "layer": layer1, 
            "mag": 1 
        ...
        edge1=pya.DEdge(len(textstr)*0.6,0,len(textstr)*0.6,0.7).transformed(DCplxTrans)
