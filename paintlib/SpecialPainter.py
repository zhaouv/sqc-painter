...
class SpecialPainter:
    ...
    def ConnectionOnPainter(self, clength=30000, cwid=54000, widout=114000, linewid=5000, slength1=16000, slength2=16000, clengthplus=0, turningRadiusPlus=5000, reverse=False):
        ...
        polygons = SpecialPainter.Connection(brush, widin=brush.widin, widout=widout, linewid=linewid, slength1=slength1,
                                             slength2=slength2, clength=clength, cwid=cwid, clengthplus=clengthplus, turningRadiusPlus=turningRadiusPlus)
    ...
    def DrawParametricCurve(cell, layer, brush, xfunc, yfunc, pointnumber, startlength, deltalength, number, lengthlist):
        ...
        def getp(ll, p1, p2):
            ...
            return pya.DPoint(p1.x+k*dx, p1.y+k*dy)
        #
        cpts = [pya.DPoint(xfunc(ii/(pointnumber-1)), yfunc(ii/(pointnumber-1)))
        ...
        p0 = cpts[0]
        p1 = cpts[1]
        brush0 = CavityBrush(pointc=p0, angle=atan2(
            p0.y-p1.y, p0.x-p1.x), widout=brush.widout, widin=brush.widin, bgn_ext=0)
        p0 = cpts[-1]
        p1 = cpts[-2]
        brush1 = CavityBrush(pointc=p0, angle=atan2(
            p0.y-p1.y, p0.x-p1.x), widout=brush.widout, widin=brush.widin, bgn_ext=0)
        return [brush0, brush1]

    @staticmethod
    def _boxes_move_and_copy(inregion, radius, number):
        xys = [(radius*cos(2*pi*ii/number), radius*sin(2*pi*ii/number))
    ...
    def DrawFillRegion(cell, layer, radius, number, layerlist=None, layermod='not in', box=None, cutbool=True):
        ...
        outregion, inregion = Collision.getShapesFromCellAndLayer(
            cellList=[IO.top], layerList=layerlist, box=box, layermod=layermod)
    ...
    def contortion(x, y, angle, width, height, length, radius, widout=20000, widin=10000, strategy='width', infoOnly=False):
        ...
        '''
        def getbrush():
            painter = CavityPainter(pointc=pya.DPoint(
                x, y), angle=angle+180, widout=widout, widin=widin, bgn_ext=0, end_ext=0)
        ...
        if n % 2 == 0:
            ...
            path += 'r{r} s{dl} l{r} '
        path += 's{s1}'
        path = path.format(s1=(width-(n+1)*2*radius)/2, r=radius,
                           dl=dl, dl2=dl*2+radius*2, nm=int(floor((n-1)/2)))
