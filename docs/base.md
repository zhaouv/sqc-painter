...
+ 方法`DrawBoxes(cell,layer,dlength,dgap,radius,number,layerlist=None,` `layermod='not in',box=None,cutbool=True,dx=0,dy=0,filterfunc=None)`  
在选定box内填充网格  
...
再`DrawBoxesInRegion(cell,layer,region,dlength,dgap,dx=0,dy=0,filterfunc=None)`重新画网格  
filterfunc为None时不检查  
`filterfunc=lambda pp:pp.area()>=20000`则只保留面积大于20000的部分, 
